...
export const getRejectedData = createAsyncThunk(
  'rejected/getRejectedData',
  async (_, { rejectWithValue, getState }) => {
    try {
      const { accessToken } = getState().auth
      const { pageNo, pageSize } = getState().rejected
...
const rejectedSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(getRejectedData.pending, (state) => {
        state.loading = true
        state.error = null
      })
      .addCase(getRejectedData.fulfilled, (state, action) => {
      ...
      .addCase(getRejectedData.rejected, (state, action) => {
        state.loading = false
