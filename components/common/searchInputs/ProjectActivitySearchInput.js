...
const ProjectActivitySearchInput = ({
  label,
  value,
  handleChange,
  placeholder,
  ...
  const fetchData = async () => {
    try {
      ...
      const url = `${MDMBaseUrlV2}/project-activity/search`
      const reqBody = {
        company: '7777',
        searchTerm: query,
        project: projectId,
      ...
      const activityList = resp.data?.data || []
      const parsedActivityList = activityList.map(
        ({ id, description, displayValue }) => {
          return {
            itemId: id,
      ...
      setFilteredData(parsedActivityList)
      setIsLoading(false)
    } catch (error) {
      console.log(error)
      setIsLoading(false)
...
export default ProjectActivitySearchInput
